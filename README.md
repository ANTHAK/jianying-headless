...
这是公开的源码预览，主版本为 **11.5.0**，兼容 **11.4.2**；不是剪映官方 SDK，也不是无需安装剪映的独立剪辑软件。
官方程序库、账号资料、原工程素材库与效果资源均不随仓库分发；下方仅展示经授权的案例预览。

## 看真实案例：AI 做完，还能接着剪

之前那条约 50 秒的 IG 教程，不是作为一整段视频塞进剪映：人物、插入画面、配音和文字被拆成
**23 条轨道、154 段内容，其中 109 段文字可继续编辑**。剪映 11.5.0 已实际导出完整 **1507 帧**。

| Hypit 原成片 | 剪映导入后原生导出 |
| --- | --- |
| [![Hypit 原成片六帧](docs/media/hypit-original-frames.png)](docs/media/hypit-original-preview.mp4) | [![剪映导出六帧](docs/media/jianying-import-frames.png)](docs/media/jianying-import-preview.mp4) |
| [观看约 50 秒预览](docs/media/hypit-original-preview.mp4) | [观看约 50 秒预览](docs/media/jianying-import-preview.mp4) |

两侧均为真实 IG 案例，没有使用合成测试图案。视频为保留全部帧与音轨的网页压缩版，图片取自相同六个时刻。
**这是可编辑工程交接，不是视觉无损转换**：字体、裁切和个别插入画面的显示时机仍有差异，
例如第 37 秒两侧内容不同。见[预览来源与处理说明](docs/media/README.md)。
...
Skill 是调用入口，不包含剪映引擎。单独安装 Skill 后仍需检出核心代码，并安装匹配版本的剪映。
...
| 原生 MP4 | 已验证快照、11.5.0 主版本 / 11.4.2 兼容版本、H.264/AAC；不自动导出用户后来手改的内容 |
...
Hypit 准备画面、配音和时间安排，转换后交给剪映继续换镜头、改字幕、调声音。
不是把最终视频简单塞进剪映，也不是把任意 Hypit 工程无损转换成剪映工程。

已有约 **50.23 秒**的 IG 教程案例：**39 份素材、23 条轨道、154 段内容，其中 109 段文字可编辑**。
原案例在 11.4.2 完成首页登记并打开；2026-09-19 从原归档核对素材，在 11.5.0 重新构建、登记、
原生界面播放至结尾，并通过原生导出与完整解码检查。保存后冷重开及完整主观视听验收仍待完成。
特殊花字、逐词颜色动画、部分裁切/阴影尚不能原样保留；不将可编辑等同于画面完全一致。

见 [Hypit 协作案例与交接方法](docs/HYPIT-COLLABORATION.md)，以及
[最小交接计划](examples/hypit-handoff.plan.json)。当前是单向、按项目转换，不是通用一键导出器或双向同步。

## 环境

- Apple Silicon Mac、macOS 26.0+；固定 codec 的最低系统版本为 26.0，本次实测为 26.5.1。
- 自行安装的剪映专业版：**11.5.0 为主版本，11.4.2 为兼容版本**；按实际安装身份选择独立配置。11.4.0 仅保留历史草稿配置，不支持原生导出。
...
该目录仅包含 Skill、调用入口和口播辅助脚本；本核心项目也已公开，但官方引擎和资源不随源码分发。
